...
const TaskCommentsComponent = ({boardID, userRole}) => {
    ...
    const [createComment, {
        isLoading: isLoadingCreateTask, isSuccess: commentCreated, isError: createCommentError, error,
    },] = useCreateCommentMutation();
    const {data, isLoading: commentsLoading} = useGetTaskCommentsQuery({
        id: taskComments.payload.taskID, page,
    });
    const [deleteTaskComment, {
        isLoading: isDeletingCommentLoading, isSuccess: succesed, isError: deleteError,
    ...
    function createNewComment() {
        const payload = {
            comment: comment, task_id: taskComments.payload.taskID, board_id: boardID, tagged_user_email: searchUser,
        };
    ...
    useEffect(() => {
        ...
        if (result) {
            ...
            const getEmailFromResult = result.input.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/);
                ...
                <div className={classes.addComentMainContainer}>
                    <div className={classes.addCommentContainer}>
                        <input
                            ...
                            value={comment}
                        />
                        {isLoadingCreateTask ? (<div className={classes.createTaskLoading}>
                                <LoadingSpinner width={"1.5rem"} height={"1.5rem"}/>
                            </div>) : (<button onClick={createNewComment}>✚</button>)}
                    </div>
                    {appError && (<div className={classes.appError}>
                            <p>{appError}</p>
                        </div>)}
                </div>

                <div className={classes.commentsPanel}>
                    {commentsLoading ? (<div className={classes.loadingContainer}>
                            <LoadingSpinner/>
                        </div>) : (<>
                            {data && data.data.comments.length === 0 ? (<div className={classes.noContent}>
                                    <h3>Nothing to display</h3>
                                </div>) : (<div className={classes.commentsContainer}>
                                    {data && data.data.comments.map((comment) => {
                                        return (<div key={comment.id} className={classes.comment}>
                                                {comment.id === requestDelete && isDeletingCommentLoading ? (
