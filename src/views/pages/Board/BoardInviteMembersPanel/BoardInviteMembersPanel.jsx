...
import { useSendBoardInviteMutation } from '../../../../state/sendBoardInvite/api';
...
const BoardInviteMembersPanel = ({boardID}) => {
  ...
  const [sendBoardInvite, {isLoading: isBoardInviteLoading, isSuccess: hasBoardInviteSended, isError: sendBoardInviteError, error}] = useSendBoardInviteMutation();
  const closePanel = () => {
    dispatch(setBoardInvitePanelActive({
        isPanelActive: false,
    }))
  }
  const sendInvite = () => {  
           ...
           <div className={classes.content}>
            <div className={classes.panelHeader}>
                ...
                <div className={classes.inviteUserContainer}>
                    <input placeholder='Enter user email' onChange={(e) => setUserEmail(e.target.value)}/>
                    <div>
                    {isBoardInviteLoading ? <LoadingSpinner width={"1.5rem"} height={"1.5rem"}/> : <button onClick={sendInvite}>Send invite</button>}
                    </div>
                </div>
                {sendInviteMsg && <p className={classes.appMsg}>{sendInviteMsg}</p>}
                <div className={classes.anno}>
                    <p><strong>Note:</strong> Invited users don't neccesary need an account.</p>
                </div>
           </div>
        </div>
    </div>
